...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateSIPUserId(IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/,
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateSIPUserId(IN const AString &strDialString,
        ...
        IN const AString &strPhoneContext /* = AString::ConstNull() */)
{
    ...
    objURI.Append('@');
    objURI.Append((strPhoneContext.GetLength() == 0) ?\
            GetHomeDomainName(nSlotId) : strPhoneContext);
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateSIPUserIdWithDialString(IN const AString &strDialString,
        IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/,
        IN const AString &strPhoneContext /* = AString::ConstNull() */)
{
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateSIPUserIdWithPhone(IN const AString &strDialString,
        IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/,
        IN const AString &strPhoneContext /* = AString::ConstNull() */)
{
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateTelUserId(IN const AString &strPhoneContext,
        IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/)
{
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateTelUserId(IN const AString &strDialString,
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateTemporaryHomeDomainName(IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/)
{
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateTemporaryPrivateUserId(IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/)
{
    ...
    strTIMPI.Sprintf("%s@ims.mnc%s.mcc%s.3gppnetwork.org",
            strIMSI.GetStr(), strMNC.GetStr(), strMCC.GetStr());
...
*/
PUBLIC GLOBAL
AString ImsIdentity::CreateTemporaryPublicUserId(IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/)
{
    ...
    strTIMPU.Sprintf("sip:%s@ims.mnc%s.mcc%s.3gppnetwork.org",
            strIMSI.GetStr(), strMNC.GetStr(), strMCC.GetStr());
...
*/
PUBLIC GLOBAL
const AString& ImsIdentity::GetAnonymousUserId()
{
...
*/
PRIVATE GLOBAL
const AString& ImsIdentity::GetHomeDomainName(
        IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/,
        IN const AString &strSubscriberId /* = AString::ConstNull() */)
{
    ...
    if (strSubscriberId.GetLength() == 0)
    {
        ...
        pSubsConfig = ConfigurationManager::GetInstance()->GetSubscriberConfig(
                strId, nSlotId);
    }
    else
    {
        pSubsConfig = ConfigurationManager::GetInstance()->GetSubscriberConfig(
                strSubscriberId, nSlotId);
    }
...
*/
PUBLIC GLOBAL
IMS_BOOL ImsIdentity::GetMccMnc(IN const AString &strPLMN, IN IMS_SINT32 nMncDigits,
...
*/
PUBLIC GLOBAL
const AString ImsIdentity::GetPhoneContext(IN IMS_SINT32 nDialingPolicy,
        IN IMS_SINT32 nSlotId /* = IMS_SLOT_0*/,
    ...
    if (nDialingPolicy == DIALING_POLICY_GEO_LOCAL)
    {
        ...
        else
        {
                    ...
                    || (pANI->nType == AccessNetworkInfo::TYPE_3GPP_E_UTRAN_TDD))
            {
                ...
                {
                    strMcc.Sprintf("%x%x%x",
                            (pAI->aPLMNId[0] >> 4) & 0x0F,
                            pAI->aPLMNId[0] & 0x0F,
                    ...
                    if ((pAI->aPLMNId[2] & 0x0F) == 0x0F)
                    {
                        strMnc.Sprintf("%x%x",
                                pAI->aPLMNId[1] & 0x0F,
                                (pAI->aPLMNId[2] >> 4) & 0x0F);
                    }
                    else
                    {
                        strMnc.Sprintf("%x%x%x",
                                pAI->aPLMNId[1] & 0x0F,
                                (pAI->aPLMNId[2] >> 4) & 0x0F,
                                pAI->aPLMNId[2] & 0x0F);
                    }
                ...
                if ((strMcc.GetLength() != 0) && (strMnc.GetLength() != 0))
                {
                    strPC.Sprintf("%s.%s.eps.%s",
                            strMcc.GetStr(), strMnc.GetStr(), strHDN.GetStr());
                }
            }
                    ...
                    || (pANI->nType == AccessNetworkInfo::TYPE_IEEE_802_11N))
            {
                ...
                else
                {
                    strMac.Sprintf("%02x%02x%02x%02x%02x%02x",
                            pAI->aMAC[0], pAI->aMAC[1], pAI->aMAC[2],
                            pAI->aMAC[3], pAI->aMAC[4], pAI->aMAC[5]);
                }
                ...
                {
                    strPC.Sprintf("%s.%s.i-wlan.%s",
                            strSsid.GetStr(), strMac.GetStr(), strHDN.GetStr());
                }
            }
                    ...
                    || (pANI->nType == AccessNetworkInfo::TYPE_3GPP_UTRAN_TDD))
            {
                        ...
                        || (pANI->nType == AccessNetworkInfo::TYPE_3GPP_GERAN))
                {
                    ...
                    if (pAI->acCGI[0] != '\0')
                    {
                        ...
                        IMS_SINT32 nMncDigits =
                                (strAI.GetLength() == (ANI_3GPP_CGI_MAX_TOTAL_LEN - 1))\
                                ? 2 : 3;
...
*/
PUBLIC GLOBAL
const AString& ImsIdentity::GetUnavailableUserId()
{
