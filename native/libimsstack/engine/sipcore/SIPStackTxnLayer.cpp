...
LOCAL SIP_BOOL SIPStackTxnLayer_FetchTransaction(IN SIP_VOID* pvTxnKey,
        IN SIP_INT32 nOption, OUT SIP_VOID** ppvOutTxnKey, OUT SIP_VOID** ppvTxn)
{
    ...
    if (pStackState->FetchTransaction(
            static_cast<SipTxnKey*>(pvTxnKey),
            nOption,
            pOutKey,
            pOutTxn))
    {
...
LOCAL SIP_BOOL SIPStackTxnLayer_ReleaseTransaction(IN SIP_VOID* pvInTxnKey,
        IN SIP_INT32 nOption, OUT SIP_VOID** ppvOutTxnKey, OUT SIP_VOID** ppvTxn)
{
    SipTxnKey* pOutKey = IMS_NULL;
    SipTxn* pOutTxn = IMS_NULL;
    ...
    if (pStackState->ReleaseTransaction(
            static_cast<SipTxnKey*>(pvInTxnKey),
            nOption,
            pOutKey,
            pOutTxn))
    {
...
LOCAL SIP_VOID* SIPStackTxnLayer_CreateAckRequest(IN SIP_VOID* pvRespMsg,
        IN ISipUserData* pUserData)
{
    ...
    SIPTxnContextData *pTCD = (pTC != IMS_NULL) ?\
            reinterpret_cast<SIPTxnContextData*>(pTC->pTxnContextData) : IMS_NULL;
    SIPClientTransactionState* pCTState = (pTCD != IMS_NULL) ?\
            reinterpret_cast<SIPClientTransactionState*>(pTCD->GetTxnState()) : IMS_NULL;
    ...
    return reinterpret_cast<void*>(pCTState->CreateAckRequest(
            reinterpret_cast<SipMessage*>(pvRespMsg)));
}

LOCAL SIP_VOID SIPStackTxnLayer_PreProcessMessageSentByStack(IN SIP_VOID* pvSipMsg,
        IN ISipUserData* pUserData)
{
    SipTxnContext* pTC = reinterpret_cast<SipTxnContext*>(pUserData->GetUserData());
    SIPTxnContextData *pTCD = (pTC != IMS_NULL) ?\
...
LOCAL SIP_VOID SIPStackTxnLayer_OnTimerExpired(IN ISipUserData *pUserData,
        IN IMS_SINT32 nTimerType)
{
    ...
    */
    ...
    if ((enTimerType == ETXN_TIMERD)
            || (enTimerType == ETXN_TIMERI)
            || (enTimerType == ETXN_TIMERJ)
            || (enTimerType == ETXN_TIMERK))
    {
