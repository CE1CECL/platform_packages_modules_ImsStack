...
public:
    ...
    IMS_BOOL FormContactFeatures(IN IMS_SINT32 nSIPMethod, IN IMS_BOOL bRequest,
            OUT AString &strContactFeatures);
    void UpdateRegCaps(IN CallerCapability *pRegCaps);

private:
    CallerCapability* GetExcludedFeaturesForRegCaps(
            IN IMS_BOOL bCreate = IMS_FALSE);
    CallerCapability* GetFeaturesForAllMessage(
            IN IMS_BOOL bCreate = IMS_FALSE);
    CallerCapability* GetFeaturesForRequest(IN IMS_SINT32 nSIPMethod,
            IN IMS_BOOL bCreate = IMS_FALSE);
    CallerCapability* GetFeaturesForResponse(IN IMS_SINT32 nSIPMethod,
            IN IMS_BOOL bCreate = IMS_FALSE);
