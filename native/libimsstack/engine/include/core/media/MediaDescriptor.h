/*
    ...
    - RTP/AVP (IStreamMedia) : rtpmap, fmtp, dccp-service-code, rtcp-mux, rtp-fb, ptime,
...
private:
    ...
    virtual IMSList<AString> GetAttributes(IN IMS_SINT32 nType,
            IN CONST AString &strType = AString::ConstNull()) const;
    ...
    virtual const SdpPrecondition* GetPrecondition(IN IMS_SINT32 nAttribute,
            IN IMS_SINT32 nType = SdpPrecondition::TYPE_QOS) const;
    virtual IMS_RESULT RemovePrecondition(IN IMS_SINT32 nAttribute,
            IN IMS_SINT32 nType = SdpPrecondition::TYPE_QOS);
    virtual IMS_RESULT SetPrecondition(IN IMS_SINT32 nAttribute,
            IN CONST SdpPrecondition *pPrecondition);

    static IMS_BOOL IsAttributeReserved(IN CONST SdpMediaParameter *pMediaParam,
            IN CONST AString &strAttribute);

private:
    // Maximum number of reserved attributes
    enum { MAX_RESERVED_ATTRIBUTE = 37 };
