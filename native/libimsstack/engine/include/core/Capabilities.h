...
public:
    ...
    //// IMS extensions
    IMS_RESULT Accept(IN IMS_BOOL bFeatureInContact = IMS_TRUE,
...
private:
    IMS_BOOL CreateContactHeader(OUT AString &strContactHeader,
    ...
    static void CollectSDPFieldsFromRegistry(IN CONST AppConfig *pAppConfig,
            IN IMS_BOOL bRequest, IN IMS_SINT32 nSector, IN_OUT SdpDescription &objSDPDesc);
    static void CopySDPFields(IN CONST SdpDescription &objSDPDesc,
            IN_OUT SdpDescription &objConcreteSDPDesc);
    static void SetSDPFields(IN CONST AStringArray &objSDPLines,
