...
#ifndef _SIP_SIP_STACK_H_
#define _SIP_SIP_STACK_H_
...
#define SipMessage SipMessage
#define SipHeader SipHeaderBase
#define SipTxnKey SipTxnKey
#define SipTxnBuffer SipTxn
#define SipAddrSpec SipAddrSpec
#define SipAddrSipUri SipUri
#define SipMsgBody SipMsgBody
#define SipError SipEn_ErrorTypes
#define SipTranspParm SipTranspParm

#define SipTxnTimeoutData SipTimeoutData
#define SipTimerType SipEn_TimerType
#define SipTimerCallback SIPCB_TIMERHANDLER
...
// Enumerations
    ...
    {
        if (pObject != IMS_NULL) pObject->increment();//(pObject->m_usRefCount)++;
    }
    template <typename T>
    inline void RemoveReference(IN_OUT T &pObject)
    {
        if (pObject != IMS_NULL) pObject->decrement();//(pObject->m_usRefCount)--;
    }
    template <typename T>
    ...
    GLOBAL SipHeader* CreateViaHeader(IN CONST AString &strSentProtocol,IN CONST AString &strSentBy,
        IN CONST AString &strBranch);
    ...
    GLOBAL SipAddrSpec* GetAddrSpec(IN SipMessage *pstMessage, IN IMS_SINT32 nType, IN IMS_UINT32 nIndex = 0);
    ...
    GLOBAL SipHeader* GetHeader(IN SipMessage *pstMessage, IN IMS_SINT32 nType, IN IMS_UINT32 nIndex = 0);
    ...
    GLOBAL IMS_BOOL GetHostAndPort(IN SipAddrSpec *pstAddrSpec, OUT AString &strHost, OUT IMS_UINT32 &nPort);
    ...
    GLOBAL AString GetMIMEHeader(IN SipMsgBody *pstMsgBody, IN IMS_SINT32 nType, IN IMS_SINT32 nIndex = 0);
    GLOBAL IMS_SINT32 GetMIMEHeaderCount(IN SipMsgBody *pstMsgBody, IN IMS_SINT32 nType);
    GLOBAL AString GetParameter(IN SipAddrSpec *pstAddrSpec, IN CONST AString &strName, IN IMS_UINT32 nIndex = 0);
    GLOBAL AString GetParameter(IN SipHeader *pstHeader, IN CONST AString &strName, IN IMS_UINT32 nIndex = 0);
    ...
    GLOBAL IMS_BOOL SetParameter(IN SipHeader* pstHeader, IN CONST AString &strName, IN CONST AString &strValue);
    ...
    {
        SipEn_TxnType enTxnType = static_cast<SipEn_TxnType>(pstTxnBuffer->GetTxnType());
