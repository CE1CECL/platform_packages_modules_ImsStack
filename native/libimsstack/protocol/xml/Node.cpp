...
PUBLIC VIRTUAL
INode* Node::AppendChild(IN INode* piChild)
{
    if (piChild == IMS_NULL)
    {
        return IMS_NULL;
...
PUBLIC VIRTUAL
INode* Node::CloneNode(IN IMS_BOOL bDeep)
{
    (void)bDeep;
    return IMS_NULL;
}
...
PUBLIC VIRTUAL
INamedNodeMap* Node::GetAttributes() const
{
...
PUBLIC VIRTUAL
INodeList* Node::GetChildNodes() const
{
...
PUBLIC VIRTUAL
INode* Node::GetFirstChild() const
{
    return m_pNodeList->Item(0);
}

PUBLIC VIRTUAL
INode* Node::GetLastChild() const
{
    return m_pNodeList->Item(m_pNodeList->GetLength() - 1);
}

PUBLIC VIRTUAL
const AString& Node::GetLocalName() const
{
    return m_strLocalName;
}

PUBLIC VIRTUAL
const AString& Node::GetNameSpaceUri() const
{
    return m_strNameSpaceUri;
}

PUBLIC VIRTUAL
INode* Node::GetNextSibling() const
{
    return m_piNextNode;
}

PUBLIC VIRTUAL
const AString& Node::GetNodeName() const
{
    return m_strNodeName;
}

PUBLIC VIRTUAL
IMS_SINT32 Node::GetNodeType() const
{
    return m_nNodeType;
}

PUBLIC VIRTUAL
const AString& Node::GetNodeValue() const
{
    return m_strNodeValue;
}

PUBLIC VIRTUAL
IDocument* Node::GetOwnerDocument() const
{
    return m_piOwnerDocument;
}

PUBLIC VIRTUAL
INode* Node::GetParentNode() const
{
    return m_piParentNode;
}

PUBLIC VIRTUAL
const AString& Node::GetPrefix() const
{
    return m_strPrefix;
}

PUBLIC VIRTUAL
INode* Node::GetPreviousSibling() const
{
    return m_piPreviousNode;
}

PUBLIC VIRTUAL
const AString& Node::GetTextContent() const
{
    return m_strTextContent;
}

PUBLIC VIRTUAL
IMS_BOOL Node::HasAttribute() const
{
    return (m_pNamedNodeMap->GetLength() != 0) ? IMS_TRUE : IMS_FALSE;
}

PUBLIC VIRTUAL
IMS_BOOL Node::HasChildNode() const
{
    return (m_pNodeList->GetLength() != 0) ? IMS_TRUE : IMS_FALSE;
}

PUBLIC VIRTUAL
INode* Node::InsertBefore(IN INode* piNewChild, IN INode* piRefChild)
{
    (void)piNewChild;
    (void)piRefChild;
    return IMS_NULL;
...
PUBLIC VIRTUAL
IMS_BOOL Node::IsSupported(IN const AString& strFeature, IN const AString& strVersion)
{
    (void)strFeature;
    (void)strVersion;
    return IMS_FALSE;
...
PUBLIC VIRTUAL
IMS_RESULT Node::Normalize()
{
    return IMS_SUCCESS;
}

PUBLIC VIRTUAL
INode* Node::RemoveChild(IN INode* piChild)
{
    (void)piChild;
    return IMS_NULL;
}
...
PUBLIC VIRTUAL
INode* Node::ReplaceChild(IN INode* piNewChild, IN INode* piOldChild)
{
    (void)piNewChild;
    (void)piOldChild;
    return IMS_NULL;
...
PUBLIC VIRTUAL
void Node::SetNodeValue(IN const AString& strNodeValue)
{
    m_strNodeValue = strNodeValue;
}

PUBLIC VIRTUAL
void Node::SetPrefix(IN const AString& strPrefix)
{
    m_strPrefix = strPrefix;
}

PUBLIC VIRTUAL
void Node::SetTextContent(IN const AString& strTextContext)
{
...
PUBLIC VIRTUAL
void Node::SetNextSibling(IN INode* piNode)
{
    m_piNextNode = piNode;
}

PUBLIC VIRTUAL
void Node::SetPreviousSibling(IN INode* piNode)
{
    m_piPreviousNode = piNode;
}

PUBLIC VIRTUAL
void Node::SetParent(IN INode* piNode)
{
    m_piParentNode = piNode;
}

PUBLIC VIRTUAL
IMS_RESULT Node::SetChildren(IN INode* piNode)
{
    return m_pNodeList->AddNode(piNode);
}

PUBLIC VIRTUAL
void Node::SetOwnerDocument(IN IDocument* piDocument)
{
    m_piOwnerDocument = piDocument;
}

PROTECTED GLOBAL
IMS_SINT32 Node::ConvertXmlNodeType(IN IMS_SINT32 nXmlNodeType)
{
    switch (nXmlNodeType)
    {
