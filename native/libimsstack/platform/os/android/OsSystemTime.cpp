...
PRIVATE GLOBAL
const IMS_CHAR* OsSystemTime::MONTH[] =
{
    "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"
};

PRIVATE GLOBAL
const IMS_CHAR* OsSystemTime::DAY_OF_WEEK[] =
{
...
PUBLIC VIRTUAL
void OsSystemTime::GetDate(OUT IMS_SINT32& nYear, OUT IMS_SINT32& nMonth,
...
PUBLIC VIRTUAL
void OsSystemTime::GetLocalTime(OUT IMS_UINT32& nHour, OUT IMS_UINT32& nMinute,
...
PUBLIC VIRTUAL
IMS_UINT32 OsSystemTime::GetRandom(IN IMS_BOOL bSeed /*= IMS_TRUE*/,
        IN IMS_UINT32 nRange /*= 0*/) const
{
    if (bSeed)
    {
        srand(time(NULL));
...
PUBLIC VIRTUAL
IMS_UINT32 OsSystemTime::GetTickCount() const
{
...
PUBLIC VIRTUAL
IMS_UINT32 OsSystemTime::GetTimeInSeconds() const
{
    struct timeval stTimeval = { 0, 0 };
...
PUBLIC VIRTUAL
IMS_UINT32 OsSystemTime::GetTimeInMicroSeconds() const
{
...
PUBLIC VIRTUAL
IMS_UINT32 OsSystemTime::GetTimeInMilliSeconds() const
{
...
PUBLIC VIRTUAL
AString OsSystemTime::GetTimeString() const
{
    time_t now = 0;
    struct tm stTm = STRUCT_TM_INIT;
    AString strTime;
...
PUBLIC VIRTUAL
AString OsSystemTime::GetTimeStringEx() const
{
    AString strTime;
    time_t now = 0;
    ...
    if (pTm != IMS_NULL)
    {
        strTime.Sprintf("%4d%2.2d%2.2d%2.2d%2.2d%2.2d", \
                pTm->tm_year + 1900, \
                pTm->tm_mon + 1, \
                pTm->tm_mday, \
                pTm->tm_hour, \
...
AString OsSystemTime::GetGMTimeString() const
{
    ...
    if (pstTm != IMS_NULL)
    {
        ...
        strTime.Sprintf("%d-%2.2d-%2.2dT%2.2d:%2.2d:%2.2d%s%sZ", \
                pstTm->tm_year + 1900, \
                pstTm->tm_mon + 1, \
                pstTm->tm_mday, \
                pstTm->tm_hour, \
...
AString OsSystemTime::GetUTCFormat(IN IMS_BOOL bNumOffset /*= IMS_FALSE*/) const
{
    ...
    if (!bNumOffset)
    {
        ...
        if (pstTm != IMS_NULL)
        {
            strUtcFormat.Sprintf("%d-%2.2d-%2.2dT%2.2d:%2.2d:%2.2dZ", \
                    pstTm->tm_year + 1900, \
                    pstTm->tm_mon + 1, \
                    pstTm->tm_mday, \
                    pstTm->tm_hour, \
...
PUBLIC VIRTUAL
void OsSystemTime::Sleep(IN IMS_UINT32 nMilliSeconds)
{
    if (nMilliSeconds == 0)
    {
        return;
