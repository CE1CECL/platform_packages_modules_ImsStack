...
public:
    ...
    inline AString& operator+=(IN CONST AString &objStr)
    { return Append(objStr); }
    inline AString& operator+=(IN CONST IMS_CHAR ch)
    { return Append(ch); }
    inline AString& operator+=(IN CONST IMS_CHAR *pszValue)
...
public:
    ...
    { return (pData == &SHARED_NULL); }

    inline IMS_CHAR* GetStr()
    { Detach(); return pData->pValue; }
    inline const IMS_CHAR* GetStr() const
    { return pData->pValue; }
    ...
    AString AlignLeft(IN IMS_SINT32 nWidth, IN IMS_CHAR cFill = ' ',
            IN IMS_BOOL bTruncate = IMS_FALSE) const;
    AString AlignRight(IN IMS_SINT32 nWidth, IN IMS_CHAR cFill = ' ',
            IN IMS_BOOL bTruncate = IMS_FALSE) const;
