...
{
public:
    inline IMSList() : IMSVector<T>() { }
    inline IMSList(IN CONST IMSList<T> &objRHS) : IMSVector<T>(objRHS) { }
...
public:
    ...
    { return IMSVector<T>::GetAt(nIndex); }
    inline T GetValueAt(IN IMS_UINT32 nIndex) const
    { return IMSVector<T>::GetValueAt(nIndex); }
