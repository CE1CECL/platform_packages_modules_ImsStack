...
template <typename T> struct TRAIT_TRIVIAL_ASSIGN
{
    enum
    {
        VALUE = IMS_FALSE
...
template<> struct TRAIT_TRIVIAL_ASSIGN< T > \
{ \
    enum \
    { \
        VALUE = IMS_TRUE \
...
IMS_SINT32 STRICTLY_ORDER_TYPE(const T &lhs, const T &rhs)
{
    return (lhs < rhs) ? 1 : 0;
}

template<typename T> inline
IMS_SINT32 COMPARE_TYPE(const T &lhs, const T &rhs)
{
...
template<typename T> inline
void CONSTRUCT_TYPE(T *p, IMS_SIZE_T n)
{
...
template<typename T> inline
void DESTRUCT_TYPE(T *p, IMS_SIZE_T n)
{
...
template<typename T> inline
void COPY_TYPE(T *d, const T *s, IMS_SIZE_T n)
{
...
template<typename T> inline
void ASSIGN_TYPE(T *d, const T *s, IMS_SIZE_T n)
{
...
template<typename T> inline
void SET_TYPE(T *d, const T *what, IMS_SIZE_T n)
{
    if (!TRAITS<T>::HAS_TRIVIAL_COPY)
    {
        while (n--)
...
template<typename T> inline
void MOVE_FORWARD_TYPE(T *d, const T *s, IMS_SIZE_T n = 1)
{
...
template<typename T> inline
void MOVE_BACKWARD_TYPE(T *d, const T *s, IMS_SIZE_T n = 1)
{
    if (!TRAITS<T>::HAS_TRIVIAL_COPY || !TRAITS<T>::HAS_TRIVIAL_DTOR)
    {
        while (n--)
...
struct KEY_VALUE_PAIR_T
{
    ...
    KEY_VALUE_PAIR_T() { }
    KEY_VALUE_PAIR_T(const KEY_VALUE_PAIR_T &rhs) : key(rhs.key), value(rhs.value) { }
