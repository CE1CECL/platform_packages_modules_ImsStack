...
PUBLIC VIRTUAL
int JniUceServiceThread::SetCallback(IN IMS_UINTP nNativeObj, IN CBServiceNoti pCBServiceNoti) {
...
PROTECTED VIRTUAL
IMS_BOOL JniUceServiceThread::Initialize() {
    IMS_TRACE_I("Initialize : ", 0, 0, 0);
    return IMS_TRUE;
}

PROTECTED VIRTUAL
void JniUceServiceThread::Uninitialize() {
    IMS_TRACE_I("Uninitialize : ", 0, 0, 0);
}

PROTECTED VIRTUAL
IMS_BOOL JniUceServiceThread::OnStart(IN IMSMSG &objMSG) {
    IMS_TRACE_I("OnStart : %d", objMSG.GetName(), 0, 0);
    return IMS_TRUE;
}

PROTECTED VIRTUAL
IMS_BOOL JniUceServiceThread::OnTerminate(IN IMSMSG &objMSG) {
    IMS_TRACE_I("OnTerminate : %d", objMSG.GetName(), 0, 0);
    return IMS_TRUE;
}

PROTECTED VIRTUAL
IMS_BOOL JniUceServiceThread::OnMessage(IN IMSMSG &objMSG) {
                ...
                for (IMS_UINT32 i = 0; i < pParam->m_nCount; i++) {
                    ...
                    if (pTempContact != null) {
                        parcel.writeString16(android::String16(
                            pTempContact->m_strContact.GetStr()));
                        parcel.writeString16(android::String16(
                            pTempContact->m_strReason.GetStr()));
                        pTempContact = null;
                    } else {
