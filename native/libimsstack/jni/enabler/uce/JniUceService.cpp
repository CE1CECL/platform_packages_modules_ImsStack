    ...
    : m_nSimSlot(_nSimSlot)
{
    ...
    //---------------------------------------------------------------------------------------------
    //m_strTarget.Sprintf("%s.UceApp%02d", EnablerUtils::GetEnablerThreadName(m_nSimSlot), nSlotId);
    ...
    : m_nSimSlot(_nSimSlot)
{
    ...
    IMSProcess::GetInstance()->LoadAppThread(STR_UCE_LISTENER_THREAD_NAME,
        JniUceServiceThread::GetInstance);
    m_pJniUceServiceThread = (JniUceServiceThread *)
