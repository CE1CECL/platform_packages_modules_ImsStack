...
public:
    ...
    MediaNego(IN MediaNego* pMediaNego)
    {
        Copy(pMediaNego);
    }
    ~MediaNego();
...
public:
    ...
    IMS_BOOL NegotiateSDP(IN ISession * pSession,
...
private:
    IMSList<IMedia*> GetIMediaListFromSession(IN ISession* pSession,
            IN MEDIA_CONTENT_TYPE eMediaType);
    void SetSessionType(IN ISession * pSession);
    void SetMediaDescriptorAsNotSupported(IN IMediaDescriptor* pDescriptor,
