...
public:
    ...
    //virtual void SetHandlerListener(IN IMediaSessionHandlerListener* pIMediaSessionHandlerListener) = 0;
    //virtual IMS_BOOL    UpdateServiceType(IN MEDIA_SERVICE_TYPE eServiceType) = 0;
    //virtual void        SetAnalyzer(IN IAnalyzerCallSession* pAnalyzer) = 0;
    //virtual IMS_BOOL SetCVOSupportValue(IN IMS_UINTP nNegoID, IN IMS_BOOL bIsSupportCVOMode) = 0;
    //virtual IMS_BOOL    PauseVT() = 0;
    //virtual IMS_BOOL    PreviewStart(IN IMS_UINTP nNegoID, IN IMS_UINTP nSurface, IN IMS_UINT32 nCameraId) = 0;
    //virtual IMS_BOOL    PreviewStop(IN IMS_UINTP nNegoID) = 0;
    //virtual IMS_BOOL    IsRunning(IMS_UINTP nNegoID) = 0;
    ...
    //virtual IMS_BOOL    Start(IN IMS_UINTP nNegoID, IN IMS_SINT32 eType = START_LIVE, IN IMS_BOOL bRTPMonitor = IMS_TRUE) = 0;
    //virtual IMS_BOOL    Start(IN IMS_UINTP nNegoID, IN IMS_SINT32 eType = START_LIVE, IN MEDIA_TRANSPORT_PROTOCOL eTimeoutCheckType = MEDIA_PROTOCOL_ANY,
    //    IN IMS_SINT32 nMonitoringTimer = -1, IN MEDIA_CONTENT_TYPE eTimeoutCheckMedia = MEDIA_TYPE_AUDIOVIDEOTEXT) = 0;
    //virtual IMS_BOOL    Update(IN IMS_UINTP nNegoID, IN IMS_SINT32 eType = START_LIVE, IN IMS_BOOL bRTPMonitor = IMS_TRUE) = 0;
    //virtual IMS_BOOL    Update(IN IMS_UINTP nNegoID, IN IMS_SINT32 eType = START_LIVE, IN MEDIA_TRANSPORT_PROTOCOL eTimeoutCheckType = MEDIA_PROTOCOL_ANY,
    //   IN IMS_SINT32 nMonitoringTimer = -1, IN MEDIA_CONTENT_TYPE eTimeoutCheckMedia = MEDIA_TYPE_AUDIOVIDEOTEXT) = 0;
    //virtual IMS_BOOL    ResumeVT() = 0;
    //virtual IMS_BOOL    SendFastVideoUpdate(void) = 0; //it need to only active video session.
    //virtual IMS_BOOL    SendDTMF(IN IMS_CHAR* strSignal, IN IMS_SINT32 nDuration) = 0; //it need to only active audio session.
    //virtual void        ResetRTPTimer(void) = 0;
    //virtual void SetDRAStatsReportOption(IN IMS_UINT32 nAction) = 0;
    //virtual void SetConfirmedSession(IN IMS_BOOL bIsConfirmed) = 0; // for RTCP-Bye packet at only Confirmed Session Stop
    //virtual void SetIsConf(IN IMS_BOOL bIsConf = IMS_FALSE) = 0; // for LGU Video Confrence Call Surface Destroy Indication
