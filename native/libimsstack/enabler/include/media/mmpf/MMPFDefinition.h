...
typedef enum _eMMPFRequest
{
    ...
    MMPF_REQUEST_PRESTART, // MMPF_MODE_RX and MMPF_MEDIA_VIDEO only, start rx video without surface
...
typedef enum _eMMPFMediaSubType
{
    ...
    MMPF_MEDIASUBTYPE_REFRESHED, // for refreshing involved nodes(Initial purpose is jitter bufffer)
...
typedef struct _tMMPFGetInfoParam_CPAddr
{
    ...
    mmpf_int32          nPdpProfileNumOf3G;           // -1 (Do not use) or operator-specific value
    //eMMPFNetworkType     eNetworkType;              // distinguish 3G/LTE, U+ only => [DO NOT USE]
