...
public:
     ...
     */
    virtual void FormatRequest(IN IMS_UINT32 eMethod, IN_OUT IMessage& objRequest) = 0;

    /**
     * Format given response message before the response is sent to the remote.
     * Response code should be decided already.
     */
    virtual void FormatResponse(IN IMS_UINT32 eMethod, IN_OUT IMessage& objResponse) = 0;

    /**
     * Get information and update its state if needed from given request from the remote.
     */
    virtual void HandleRequest(IN IMS_UINT32 eMethod, IN const IMessage& objRequest) = 0;

    /**
     * Get information and update its state if needed from given response from the remote.
     */
    virtual void HandleResponse(IN IMS_UINT32 eMethod, IN const IMessage& objResponse) = 0;
};
