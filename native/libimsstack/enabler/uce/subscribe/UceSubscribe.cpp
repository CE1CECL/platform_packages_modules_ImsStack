    ...
    , m_pRetryAfterTimer(IMS_NULL)
{
    ...
    m_strXMLDocumentHelperThreadName = AString().Sprintf("UceXmlDocumentHelperThread%d",
            GetSlotId());
    LoadConfigValue();
    SetState(ON);
}
...
PROTECTED VIRTUAL
void UceSubscribe::Timer_TimerExpired(IN ITimer *piTimer)
{
...
void UceSubscribe::SubscriptionForkedNotify(IN ISubscription* piSubscription,
...
IMS_BOOL UceSubscribe::StateON_SingleSubscribeRequested(IN IMSMSG &objMsg)
{
    ...
    if (CreateSubscription(m_strRemoteUser) != IMS_TRUE) {
        IMS_TRACE_I("StateON_SingleSubscritionReq:CreateSubscription() is failed", 0, 0, 0 );
        SendSubscribeCommandErrorInd(IUUceService::COMMAND_CODE_GENERIC_FAILURE);
        SubscribeTerminated();
        return IMS_TRUE;
    }
    if (SetHeaderForSingleSubscription(GetISIPMessage()) != IMS_TRUE) {
        IMS_TRACE_I("StateON_SingleSubscritionReq:SetHeaderForSingleSubscription() is failed",
                0, 0, 0);
        SendSubscribeCommandErrorInd(IUUceService::COMMAND_CODE_GENERIC_FAILURE);
        SubscribeTerminated();
        return IMS_TRUE;
    ...
    if (SendSingleSubscribe() != IMS_TRUE) {
        IMS_TRACE_I("StateON_SingleSubscritionReq:SendSingleSubscribe() is failed", 0, 0, 0 );
        SendSubscribeCommandErrorInd(IUUceService::COMMAND_CODE_GENERIC_FAILURE);
        SubscribeTerminated();
        return IMS_TRUE;
    }
...
IMS_BOOL UceSubscribe::StateON_ListSubscribeRequested(IN IMSMSG &objMsg)
{
    ...
    if (SetHeaderForListSubscription(piSIPMessage, strListSubscriptionRequestUri) != IMS_TRUE ||
            SetContentBody(piSIPMessage, strXMLBody) != IMS_TRUE) {
        IMS_TRACE_I("SetHeaderForListSubscription() or SetContentBody() is failed", 0, 0, 0);
        SendSubscribeCommandErrorInd(IUUceService::COMMAND_CODE_GENERIC_FAILURE);
        SubscribeTerminated();
        return IMS_TRUE;
    }
...
IMS_BOOL UceSubscribe::CreateSubscription(IN CONST AString& strToURI)
{
    ...
    m_piSubscription = m_piCoreService->CreateSubscription(AString::ConstNull(), strToURI,
            "presence");
...
{
    IMS_TRACE_I("SetState:State [ %s ] -> [ %s ]", StateToString(), StateToString( m_eState ), 0);
    m_eState = _eState;
