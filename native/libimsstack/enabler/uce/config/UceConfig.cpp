/*
    ...
    --------      --------------                ----------
    20120602    saurabh31.srivastava@           Created
...
PUBLIC GLOBAL
UceConfig* UceConfig::GetInstance()
{
...
IMS_UINT32 UceConfig::GetIntValue(IN KEY_UCE_INT eKey, IN IMS_SINT32 nSimSlot)
{
    ...
    switch(eKey) {
        ...
        break;
        case KEY_EXPONENTIAL_RETRY_PUBLISH_RESPONSE_MAX_COUNT:
            nRet = objUceAssetItems->m_nExponentialRetryPublishResponseMaxCount;
        break;
    }
...
IMSVector<IMS_SINT32> UceConfig::GetExponentialRetryPublishRespTimeArray(IN IMS_SINT32 nSimSlot)
{
    ...
    IMSVector<IMS_SINT32> objExponentialRetryPublishResponseTimeSec =
        m_objAssetMap.GetValue(nSimSlot)->m_objExponentialRetryPublishResponseTimeSec;
...
IMS_UINT32  UceConfig::GetPublishRetryType(IN IMS_SINT32 nResponseCode, IN IMS_SINT32 nSimSlot)
{
    ...
    temporary = objUceAssetItems->m_objExponentialRetryPublishResponse;
    if (!temporary.IsEmpty()) {
        for (IMS_UINT32 index = 0; index < temporary.GetSize(); index++) {
            if (nResponseCode == temporary.GetAt(index)) {
...
IMS_BOOL UceConfig::IsImsRegistrationRequired(IN IMS_BOOL isPublish, IN IMS_SINT32 nResponseCode,
...
PUBLIC VIRTUAL
void UceConfig::CarrierConfig_NotifyConfigChanged(IN IMS_SINT32 nSlotId)
{
...
void UceConfig::Update(IN ICarrierConfig* piCc, IN IMS_SINT32 nSimSlot)
{
    ...
    objUceAssetItems->m_strRlsUri =
            piCc->GetString(CarrierConfig::ImsUce::KEY_RLS_URI_STRING);
    ...
    objUceAssetItems->m_objImmediatelyRetryPublishResponse =
            piCc->GetIntArray(
                CarrierConfig::ImsUce::KEY_IMMEDIATELY_RETRY_PUBLISH_RESPONSE_INT_ARRAY);
    objUceAssetItems->m_nImmediatelyRetryPublishResponseMaxCount =
            piCc->GetInt(
                CarrierConfig::ImsUce::KEY_IMMEDIATELY_RETRY_PUBLISH_RESPONSE_MAX_COUNT_INT);
    objUceAssetItems->m_objRetryPublishResponse =
            piCc->GetIntArray(
    ...
    objUceAssetItems->m_nRetryPublishResponseTimeSec =
            piCc->GetInt(
                CarrierConfig::ImsUce::KEY_FIXED_TIME_RETRY_PUBLISH_RESPONSE_TIME_SEC_INT);
    objUceAssetItems->m_objExponentialRetryPublishResponse =
            piCc->GetIntArray(
                CarrierConfig::ImsUce::KEY_VARIABLE_TIME_RETRY_PUBLISH_RESPONSE_INT_ARRAY);
    objUceAssetItems->m_nExponentialRetryPublishResponseMaxCount =
            piCc->GetInt(
                CarrierConfig::ImsUce::KEY_VARIABLE_TIME_RETRY_PUBLISH_RESPONSE_MAX_COUNT_INT);
    objUceAssetItems->m_objExponentialRetryPublishResponseTimeSec =
            piCc->GetIntArray(
                CarrierConfig::ImsUce::KEY_VARIABLE_TIME_RETRY_PUBLISH_RESPONSE_TIME_SEC_INT_ARRAY);
    objUceAssetItems->m_objReAttemptRegistrationPublishResponse =
            piCc->GetIntArray(
                CarrierConfig::ImsUce::KEY_REATTEMPT_REGISTRATION_PUBLISH_RESPONSE_INT_ARRAY);
    objUceAssetItems->m_objReAttemptRegistrationSubscribeResponse =
            piCc->GetIntArray(
