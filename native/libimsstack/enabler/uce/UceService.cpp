...
PROTECTED VIRTUAL
IMS_BOOL UceService::OnMessage( IN IMSMSG &objMSG )
{
...
PROTECTED VIRTUAL
void UceService::CoreService_PageMessageReceived(IN ICoreService* piService,
...
void UceService::CoreService_ReferenceReceived(IN ICoreService* piService,
...
void UceService::CoreService_SessionInvitationReceived(IN ICoreService* piService,
...
void UceService::CoreService_UnsolicitedNotifyReceived(IN ICoreService* piService,
...
void UceService::CoreService_CapabilityQueryReceived(IN ICoreService* piService,
...
{
    if (m_pUceSubscribeManager == IMS_NULL) {
        m_pUceSubscribeManager = new UceSubscribeManager(
                AString().Sprintf("UceSubscribeManager%d", m_nSlotId), m_piCoreService,
                m_strAppName, m_nSlotId);
    }
...
void UceService::EnableCoreService()
{
    ...
    m_piCoreService = DYNAMIC_CAST(ICoreService*,(Connector::Open("imscore",
            ImsServiceConfig::GetAppName(ImsAppId::UCE),strServiceID)));
...
IMS_BOOL UceService::SendOptionsRequest(IN IUceOptionsCmdPrm* pParam)
{
    ...
    if (m_pUceOptionsManager != IMS_NULL) {
        m_pUceOptionsManager->SendOptionsRequest(pParam->m_nKey, pParam->m_strRemoteUri,
                pParam->m_nMyCaps);
    }
    delete pParam;
    return IMS_TRUE;
...
IMS_BOOL UceService::SendOptionsResponse(IUceOptionsRespCmdPrm* pParam)
{
    ...
    if (m_pUceOptionsManager != IMS_NULL) {
        m_pUceOptionsManager->SendOptionsResponse(pParam->m_nKey, pParam->m_nResponseCode,
                pParam->m_strReason, pParam->m_nMyCaps);
    }
    delete pParam;
    return IMS_TRUE;
}

IMS_BOOL UceService::OptionsReceived(IN ICoreService* piCoreService,
        IN ICapabilities* piCapabilities)
{
    if (m_pUceOptionsManager != IMS_NULL) {
