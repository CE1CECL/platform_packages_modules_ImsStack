    ...
    , m_pUceService(IMS_NULL)
{
    ...
    Configuration::GetInstance()->SetAppConfig(ImsServiceConfig::GetAppName(ImsAppId::UCE),
        m_nSlotId);
...
PROTECTED VIRTUAL
IMS_BOOL UceApp::OnPreprocess( IN IMSMSG &objMSG )
{
    (void)objMSG;
    return IMS_FALSE;
}
...
void UceApp::ImsAosMonitor_Connected(IN IMS_UINT32 nServices, IN IMS_UINT32 nIpcan)
{
    ...
    IMS_TRACE_I("ImsAosMonitor_Connected:nServices [%08x], m_RegisteredNetwork [%d]",
