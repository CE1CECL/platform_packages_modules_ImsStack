...
------------------------------------------------------------------------------------------------- */
UcePublishManager::UcePublishManager(IN ICoreService* _piCoreService, IN CONST AString &strAppName,
        ...
        IN IMS_SINT32 nMessage)
{
    ...
    (m_nConnectedServices & CONNECTED_SERVICE_CALL_COMPOSER) == CONNECTED_SERVICE_CALL_COMPOSER) {
        if (strContactHeader.Contains(UceTag::TAG_ICSI) == IMS_FALSE && bAppendICSITag == IMS_FALSE)
        {
            bAppendICSITag = IMS_TRUE;
            strContactHeaderValue.Append(UceTag::TAG_ICSI);
    ...
    (m_nConnectedServices & CONNECTED_SERVICE_CPM_LARGEMSG) == CONNECTED_SERVICE_CPM_LARGEMSG) {
        if (strContactHeader.Contains(UceTag::TAG_ICSI) == IMS_FALSE && bAppendICSITag == IMS_FALSE)
        {
            bAppendICSITag = IMS_TRUE;
            strContactHeaderValue.Append(UceTag::TAG_ICSI);
...
{
    IMS_TRACE_D("SendPublishRequest:key [%d], extended[%d], capability[%d]", key, extended,
            capability);
...
IMS_BOOL UcePublishManager::StateIDLE_PublishRequested(IN IMSMSG &objMsg)
{
    ...
    if(pPublicationData == IMS_NULL) {
        ...
        SendPublishCommandErrorInd(pPublicationData->m_nKey,
                IUUceService::COMMAND_CODE_GENERIC_FAILURE);
...
IPublishResponseData* UcePublishManager::GetPublishResponseData(ISipMessage* piMessage)
{
    ...
    if (piMessage != IMS_NULL) {
        ...
        IMS_TRACE_D("GetPublishResponseData:StatusCode[%d], reason[%s]",
                piMessage->GetStatusCode(), piMessage->GetReasonPhrase().GetStr(), 0);
...
void UcePublishManager::SendPublishResponseInd(IMS_UINT32 nKey, IMS_SINT32 nResponseCode,
        ...
        AString eTag, IMS_BOOL bNeedToRetry)
{
...
IMS_BOOL UcePublishManager::CreatePublication()
{
    ...
    m_piPublication = m_piCoreService->CreatePublication(AString::ConstNull(), AString::ConstNull(),
                              UceNamespace::PRESENCE);
    if (m_piPublication == IMS_NULL) {
...
void UcePublishManager::SetRefreshPolicy(ISipMessage *piMessage, AString strMinExpiryValue)
{
    ...
    IMS_UINT32 nRefreshRatio = UceConfig::GetInstance()->GetIntValue(
        UceConfig::KEY_PUBLISH_REFRESH_RATIO, m_nSimSlot);
    if (nRefreshRatio == 0) {
...
IMS_BOOL UcePublishManager::SetPidfXmlBody(ISipMessage* piMessage)
{
    ...
    if (UceConfig::GetInstance()->GetBoolValue(UceConfig::KEY_ENCODE_PUBLISH_BODY, m_nSimSlot) ==
        IMS_TRUE && m_bReceivedFailResponse == IMS_FALSE) {
...
{
    if (m_nImmediatelyRetryCount >= UceConfig::GetInstance()->GetIntValue(
        UceConfig::KEY_IMMEDIATELY_RETRY_PUBLISH_RESPONSE_MAX_COUNT, m_nSimSlot)) {
        IMS_TRACE_I( "ProcessImmediatelyRetryResponseScenario:over max ImmediatelyRetryMaxCount",
                0, 0, 0);
        return IMS_FALSE;
    }
...
IMS_BOOL UcePublishManager::ProcessRetryResponseScenario()
{
    ...
    if (StartTimer(TIMER_RETRY, UceConfig::GetInstance()->GetIntValue(
...
IMS_BOOL UcePublishManager::ProcessExponentialRetryResponseScenario()
{
    ...
    IMS_UINT32 nExponentialRetryTimeSec = m_objExponentialRetryTimeSec.GetAt(m_nExponentialRetryCount);
...
{
    if (m_pPendingPublicationData != IMS_NULL) {
        SendPublishCommandErrorInd(m_pPendingPublicationData->m_nKey,
                IUUceService::COMMAND_CODE_GENERIC_FAILURE);
        delete m_pPendingPublicationData;
        m_pPendingPublicationData = IMS_NULL;
    }
