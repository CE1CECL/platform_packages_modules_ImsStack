...
{
    IMS_BOOL bRegistered = IMS_FALSE;

    switch (m_nState)
    {
        case STATE_REGISTERED: // FALL-THROUGH
        case STATE_REFRESHING: // FALL-THROUGH
        case STATE_REFRESHSTOP:
            bRegistered = IMS_TRUE;
            break;
...
{
    IMS_BOOL bRefreshing = IMS_FALSE;

    switch (m_nState)
    {
        case STATE_REFRESHING: // FALL-THROUGH
        case STATE_REFRESHSTOP:
            bRefreshing = IMS_TRUE;
            break;
...
{
    if (m_piStopRetryTimer != IMS_NULL || m_piOfflineRecoverTimer != IMS_NULL)
    {
        return IMS_TRUE;
    }
...
PROTECTED VIRTUAL
void AosRegistration::InitRetryIntervals()
{
    A_IMS_TRACE_D(REGID, "InitRetryIntervals", 0, 0, 0);

    m_objInitRetryIntervals.Clear();
    m_objRefreshRetryIntervals.Clear();
}
...
void AosRegistration::AddFeatureTagForMtc(IN IMS_UINT32 nRegFeatures, IN IMS_BOOL bFinalFeatureTag)
{
    ...
    {
        // TODO: update new API
        // IConfigurable::CP_I_FEATURE_TAG_OPTIONS
        // (voip+video : "0x03000208" , voip : "0x03000000")
    }
...
{
    A_IMS_TRACE_I(REGID,
            "CalculateActualWaitTime :: max-time(%d), base-time(%d), consecutive-failures (%d)",
...
void AosRegistration::ProcessDefaultFlowRecovery_Start(IN IMS_SINT32 /* nStatusCode */ /* = 0 */)
{
    ...
    IMS_BOOL bAwtPolicyToEachPcscf =
            (GET_N_CONFIG(m_nSlotId)->GetRegistrationActualWaitTimePolicy() ==
            CarrierConfig::Ims::AWT_POLICY_FAILURE_TO_EACH_PCSCF);

    if (!bAwtPolicyToEachPcscf)
    {
...
PROTECTED VIRTUAL
void AosRegistration::ProcessStartFailed_305()
{
...
PROTECTED VIRTUAL
void AosRegistration::ProcessUpdateFailed_305()
{
...
void AosRegistration::ProcessStartFailed_StatusCode(IN IMS_SINT32 nStatusCode)
{
    ...
    if (GET_N_CONFIG(m_nSlotId)->GetRegistrationRetrySip305CodePolicy() ==
            CarrierConfig::Assets::SIP_305_CODE_POLICY_3GPP &&
            nStatusCode == SIPStatusCode::SC_305)
    {
...
void AosRegistration::ProcessUpdateFailed_StatusCode(IN IMS_SINT32 nStatusCode)
{
    ...
    if (GET_N_CONFIG(m_nSlotId)->GetReregistrationRetrySip305CodePolicy() ==
            CarrierConfig::Assets::SIP_305_CODE_POLICY_3GPP &&
            nStatusCode == SIPStatusCode::SC_305)
    {
        ProcessUpdateFailed_305();
        return;
    }
